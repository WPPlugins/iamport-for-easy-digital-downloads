(function($) {
	...
	function order_title( item_names ) {
		if ( typeof item_names == 'string' )	return item_names;
		if ( item_names instanceof Array ) {
			if ( item_names.length == 1 )	return item_names[0];

			return item_names[0] + '외 상품(' + (item_names.length-1) + ')';
		}

		return 'EasyDigitalDownload 상품';
	}
	...
	$(document).ready(function() {
		...
		var iamport_gateways = ['iamport_card', 'iamport_samsung', 'iamport_kakao', 'iamport_phone'];
			...
			if ( jQuery.inArray(gateway, iamport_gateways) > -1 ) { // 일반 인증 결제
				...
				$.post(edd_global_vars.ajaxurl, $form.serialize() + '&action=edd_process_checkout', function(json_str) {
					...
					if ( obj && obj.payment_id ) {
						var user_code 		= $form[0]['iamport_user_code'].value,
							pay_method		= $form[0]['iamport_pay_method'].value,
							merchant_uid 	= $form[0]['iamport_merchant_uid'].value,
							buyer_name		= $form[0]['edd_last'].value + $form[0]['edd_first'].value,
							buyer_email		= $form[0]['edd_email'].value,
							buyer_tel		= $form[0]['edd_phone'] ? $form[0]['edd_phone'].value : '010-0000-0000';
						...
						var item_names = [];
						$.each( $('.edd_checkout_cart_item_title'), function(idx, elem) {
							item_names.push( $(elem).text() );
						});

						IMP.init(user_code);
						IMP.request_pay({
							amount : obj.price,
							name : order_title(item_names),
							...
							if ( rsp.success ) {
								location.href = obj.landing_url + '&imp_uid=' + rsp.imp_uid;
							} else {
								alert(rsp.error_msg);
								location.reload();
							}
				...
				// encrypt using public key
				var enc_card_number = rsa.encrypt( $form[0]['iamport_subscription_card_number'].value );
				var enc_card_expiry = rsa.encrypt( '20' + $form[0]['iamport_subscription_card_expiry_y'].value + $form[0]['iamport_subscription_card_expiry_m'].value );
				var enc_card_birth 	= rsa.encrypt( $form[0]['iamport_subscription_card_birth'].value );
				var enc_card_pwd 	= rsa.encrypt( $form[0]['iamport_subscription_card_pwd'].value );

				$form[0]['enc_iamport_subscription_card_number'].value = enc_card_number;
				$form[0]['enc_iamport_subscription_card_expiry'].value = enc_card_expiry;
				$form[0]['enc_iamport_subscription_card_birth'].value = enc_card_birth;
				$form[0]['enc_iamport_subscription_card_pwd'].value = enc_card_pwd;
